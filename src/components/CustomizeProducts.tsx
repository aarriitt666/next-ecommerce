...
const CustomizeProducts = ({ productId, variants, productOptions }: { productId: string, variants: products.Variant[], productOptions: products.ProductOption[] }) => {
    ...
    console.log('CustomizeProducts - Variants:', JSON.stringify(variants, null, 2));
    ...
    return (
                ...
                <div className='flex flex-col gap-4' key={option.name}>
                    ...
                    <ul className='flex items-center gap-3'>
                        {option.choices?.map(choice => {
                            ...
                            return option.name === "Color" ? (<li className="w-8 h-8 rounded-full ring-gray-300 relative" style={{ backgroundColor: choice.value, cursor: disabled ? "not-allowed" : "pointer" }} key={choice.description} onClick={clickHandler}>
                            ...
                            </li>) : (<li className="ring-1 ring-haodepink text-haodepink rounded-md py-1 px-4 text-sm" style={{ cursor: disabled ? "not-allowed" : "pointer", backgroundColor: selected ? "#f35c7a" : disabled ? "#fbcfe8" : "white", color: selected || disabled ? "white" : "#f35c7a", boxShadow: disabled ? "none" : "" }} onClick={clickHandler} key={choice.description}>{choice.description}</li>)
                        }
                        )}
                    </ul>
                </div>
        ...
        <ul className='flex items-center gap-3'>
            ...
            <ul className='flex items-center gap-3'>
                <li className="ring-1 ring-haodepink text-haodepink rounded-md py-1 px-4 text-sm cursor-pointer">Small</li>
                <li className="ring-1 ring-haodepink text-white bg-haodepink rounded-md py-1 px-4 text-sm cursor-pointer">Medium</li>
